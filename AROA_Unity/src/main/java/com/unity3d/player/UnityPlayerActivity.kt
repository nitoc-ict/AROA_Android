...
class UnityPlayerActivity : AppCompatActivity(), IUnityPlayerLifecycleEvents {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val viewModel = ViewModelProvider(this)[UnityPlayerViewModel::class.java]

        lifecycleScope.launch {
            viewModel.trialName.collect {
                when (it) {
                    is Result.Error -> {
                        setTrialNameText("HenokoRaceing!")
                    }
                    is Result.Loading -> {
                        setTrialNameText("Now Loading...")
                    }
                    is Result.Success -> {
                        setTrialNameText(it.data)
                    }
                }
            }
        }
        ...
        lifecycleScope.launch {
            viewModel.recordTime.collect {
                setRecordTimeText("Record:${it / 60}min ${it % 60}sec")
            }
        }

        lifecycleScope.launch {
            viewModel.currentRank.collect {
                setCurrentRankText("Rank:${it}")
            }
        }

        lifecycleScope.launch {
            viewModel.relativeDistance.collect {
                setCircleRadius((it.toFloat() / 100).toString())
            }
        }
        viewModel.fetchTrialById("")
